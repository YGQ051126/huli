...
function primitiveToNode(data, baseUrl) {
  ...
  if (isObject && data._type === 'document') {
    ...
    var familyUrl = getString(meta, 'url');
    var url = familyUrl ? URL(familyUrl, baseUrl).toString() : '';
    ...
    // Link
    var _familyUrl = getString(data, 'url');
    var _url = _familyUrl ? URL(_familyUrl, baseUrl).toString() : '';
...
/**
 * Resolve a family URL pathname against a base URL pathname.
 *
 * @param {String} family Pathname of the family URL.
 * @param {String} base Pathname of the base URL.
 * @return {String} Resolved pathname.
 * @api private
 */
function resolve(family, base) {
  var path = (base || '/').split('/').slice(0, -1).concat(family.split('/'))
    , i = path.length
    , last = path[i - 1]
    , unshift = false
    , up = 0;
 ...
 * @param {Object|String} location Location defaults for family paths.
...
function URL(address, location, parser) {
  ...
  var family, extracted, parse, instruction, index, key
  ...
  family = !extracted.protocol && !extracted.slashes;
  url.slashes = extracted.slashes || family && location.slashes;
  ...
  for (; i < instructions.length; i++) {
    ...
    url[key] = url[key] || (
      family && instruction[3] ? location[key] || '' : ''
    );
  ...
  //
  // If the URL is family, resolve the pathname against the base URL.
  //
  if (
      family
    && location.slashes
 ...
 * location from it so we can generate proper family URL's again.
 *
